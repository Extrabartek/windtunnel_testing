...
ax1.set_yticks(np.arange(0, 0.81, 0.1))
ax1.set_ylabel(r'Chordwise position measured from L.E. ($x/c$)')
ax1.set_xlabel(r'Angle of Attack (AoA)')
ax1.xaxis.set_major_formatter(StrMethodFormatter(u'{x:.1f}°'))
plt.plot(angles, medians, label=r'Test data', marker='x')
plt.plot(alpha_vis, x_tran_vis, label="XFOIL with viscosity")
plt.title(r'Chordwise position of transition  vs. Angle of attack')
plt.grid(True)
plt.legend(loc='best')
plt.tight_layout()
plt.show()
...
ax1.set_xticks(np.arange(-2.5, 21, 2.5))
ax1.set_yticks(np.arange(-0.5, 1.26, 0.25))
ax1.set_ylabel(r'Lift Coefficient ($c_l$)')
ax1.set_xlabel(r'Angle of Attack (AoA)')
ax1.xaxis.set_major_formatter(StrMethodFormatter(u'{x:.1f}°'))
# plt.plot(alpha_vis_low, cl_vis_low, label=r'50 panels')
# plt.plot(alpha_vis, cl_vis, label=r'160 panels')
# plt.plot(alpha_vis_high, cl_vis_high, label=r'494 panels')
plt.plot(alpha_test, cl_test, label=r'Test data', marker='x')
...
ax1.set_ylim(0, 0.16)
ax1.set_xticks(np.arange(-2.5, 21, 2.5))
ax1.set_yticks(np.arange(0, 0.221, 0.02))
ax1.set_ylabel(r'Drag Coefficient ($c_d$)')
ax1.set_xlabel(r'Angle of Attack (AoA)')
ax1.xaxis.set_major_formatter(StrMethodFormatter(u'{x:.1f}°'))
plt.plot(alpha_test, cd_test, marker='x', label="Test data")
# plt.plot(alpha_vis_low, cd_vis_low, label="50 panels")
plt.plot(alpha_vis, cd_vis, label="XFOIL with viscosity")
# plt.plot(alpha_vis_high, cd_vis_high, label="494 panels")
...
ax1.set_xticks(np.arange(-2.5, 21, 2.5))
ax1.set_yticks(np.arange(-0.06, 0.041, 0.01))
...
# plt.plot(alpha_vis_low, cm_vis_low, label="50 panels")
# plt.plot(alpha_vis, cm_vis, label="160 panels")
# plt.plot(alpha_vis_high, cm_vis_high, label="494 panels")
...
ax1.set_xticks(np.arange(0, 0.081, 0.01))
ax1.set_yticks(np.arange(-0.2, 1.21, 0.2))
ax1.set_ylabel(r'Lift Coefficient ($c_l$)')
ax1.set_xlabel(r'Drag Coefficient ($c_d$)')
# ax1.xaxis.set_major_formatter(StrMethodFormatter(u'{x:.1f}°'))
plt.plot(cd_test, cl_test, marker='x', label="Test data")
